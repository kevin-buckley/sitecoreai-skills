...
description: >-
  ...
  insert options, and field types - as Sitecore XM Cloud templates. Use when migrating or mapping
  templates and fields from XP to XM Cloud, deciding which legacy fields to drop, or making page
  templates inherit from both SXA Page and _Designable. Also use when the user asks about field
  mapping, standard values migration, or creating templates through MCP tooling and the authoring UI
  rather than hand-authored YAML. For auditing templates in an existing XM Cloud project rather than
  migrating into one, use sitecore-data-templates.
license: Apache-2.0

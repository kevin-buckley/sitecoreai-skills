...
description: >-
  Rebuild an XP site's structure - site and tenant definition, information architecture, navigation,
  routes, site settings, dictionary, and redirects - in Sitecore XM Cloud with the Content SDK. Use
  when migrating a site or SXA site from XP to XM Cloud, planning site-level IA or route architecture
  for a migration, or deciding which SXA site concepts should not carry over. Covers site-level
  structure only: use sitecore-template-migration for the content model, sitecore-component-migration
  for renderings, and sitecore-page-design-setup for Page Design and Partial Design wiring.
license: Apache-2.0
